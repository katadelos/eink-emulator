#!/usr/bin/env python3
"""Build a minimal Wario eMMC user-area image from the supplied artifacts."""
...
def write_userstore(disk, output_offset: int, sectors: int,
    ...
    """Create Wario's nested-MBR p4 and optionally populate its FAT32 userstore."""
    inner_start = 16
    ...
    try:
        ...
        raise ValueError("unknown Wario userstore geometry") from error
...
def build(
    ...
    *,
    ...
    """Build a sparse Wario-family eMMC user-area image."""
    try:
        partitions = PARTITION_LAYOUTS[layout]
    except KeyError as error:
        raise ValueError(f"unknown Wario partition layout: {layout}") from error
