"""Build a sparse Celeste eMMC image from a kernel and prepared rootfs."""
...
def validate(kernel: Path, rootfs: Path) -> None:
    with kernel.open("rb") as source:
        if source.read(4) != b"\x27\x05\x19\x56":
            raise SystemExit("Celeste kernel is not a legacy uImage")
    if KERNEL_OFFSET + kernel.stat().st_size > P1_START * SECTOR_SIZE:
        raise SystemExit("Celeste kernel overlaps partition 1")
    if rootfs.stat().st_size > ROOTFS_SIZE:
        raise SystemExit("Celeste rootfs exceeds partition 1")
    with rootfs.open("rb") as source:
        source.seek(EXT_MAGIC_OFFSET)
        if source.read(2) != b"\x53\xef":
            raise SystemExit("Celeste rootfs is not an ext filesystem")
...
def provision_userstore(disk: BinaryIO) -> None:
    ...
    disk.seek(USERSTORE_START * SECTOR_SIZE)
    ...
    for fat_sector in (first_fat, first_fat + USERSTORE_FAT_SECTORS):
        disk.seek(fat_sector * SECTOR_SIZE)
        disk.write(fat_prefix)


def build(output: Path, kernel: Path, rootfs: Path) -> None:
    """Build Celeste's user-area partition and nested userstore layout."""
